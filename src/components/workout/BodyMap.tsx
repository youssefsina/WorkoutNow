...
interface BodyMapProps {
  muscleList: MuscleItem[];
  selectedMuscles: string[];
  onToggleMuscle: (id: string) => void;
}

const MUSCLE_META: Record<string, { label: string; side: "front" | "back"; color: string }> = {
  chest: { label: "Chest", side: "front", color: "#ef4444" },
  abdominals: { label: "Abs", side: "front", color: "#f97316" },
  obliques: { label: "Obliques", side: "front", color: "#eab308" },
  "front-shoulders": { label: "Front Delts", side: "front", color: "#22c55e" },
  biceps: { label: "Biceps", side: "front", color: "#06b6d4" },
  forearms: { label: "Forearms", side: "front", color: "#8b5cf6" },
  quads: { label: "Quads", side: "front", color: "#ec4899" },
  calves: { label: "Calves", side: "front", color: "#f59e0b" },
  traps: { label: "Traps", side: "front", color: "#14b8a6" },
  hands: { label: "Hands", side: "front", color: "#64748b" },
  lats: { label: "Lats", side: "back", color: "#ef4444" },
  lowerback: { label: "Lower Back", side: "back", color: "#f97316" },
  "traps-middle": { label: "Mid Traps", side: "back", color: "#eab308" },
  "rear-shoulders": { label: "Rear Delts", side: "back", color: "#22c55e" },
  triceps: { label: "Triceps", side: "back", color: "#06b6d4" },
  hamstrings: { label: "Hamstrings", side: "back", color: "#8b5cf6" },
  glutes: { label: "Glutes", side: "back", color: "#ec4899" },
};

const MUSCLE_GROUPS: Record<string, string[]> = {
  "Upper Body": ["chest", "abdominals", "obliques", "front-shoulders", "biceps", "lats", "traps", "traps-middle", "rear-shoulders", "triceps", "lowerback"],
...
function FrontBody({ selected, onToggle }: { selected: Set<string>; onToggle: (id: string) => void }) {
  const active = (id: string) => selected.has(id);
  const fill = (id: string) => active(id) ? MUSCLE_META[id]?.color ?? "#ef4444" : "currentColor";
  const cls = (id: string) =>
    `cursor-pointer transition-all duration-200 ${active(id) ? "opacity-100 drop-shadow-md" : "opacity-20 hover:opacity-40"}`;

  return (
    <svg viewBox="0 0 660.46 1206.46" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-full w-auto max-h-[500px] md:max-h-[600px]">
      <defs>
        <radialGradient id="jointradial-f" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
          <stop offset="0%" style={{ stopColor: "rgb(254,91,127)", stopOpacity: 1 }} />
          <stop offset="100%" style={{ stopColor: "rgb(231,236,239)", stopOpacity: 1 }} />
        </radialGradient>
      </defs>

      <g id="calves" className={cls("calves")} onClick={() => onToggle("calves")} style={{ color: fill("calves") }}>
      ...
      <g id="quads" className={cls("quads")} onClick={() => onToggle("quads")} style={{ color: fill("quads") }}>
      ...
      <g id="abdominals" className={cls("abdominals")} onClick={() => onToggle("abdominals")} style={{ color: fill("abdominals") }}>
      ...
      <g id="obliques" className={cls("obliques")} onClick={() => onToggle("obliques")} style={{ color: fill("obliques") }}>
      ...
      <g id="forearms" className={cls("forearms")} onClick={() => onToggle("forearms")} style={{ color: fill("forearms") }}>
      ...
      <g id="biceps" className={cls("biceps")} onClick={() => onToggle("biceps")} style={{ color: fill("biceps") }}>
      ...
      <g id="front-shoulders" className={cls("front-shoulders")} onClick={() => onToggle("front-shoulders")} style={{ color: fill("front-shoulders") }}>
      ...
      <g id="chest" className={cls("chest")} onClick={() => onToggle("chest")} style={{ color: fill("chest") }}>
      ...
      <g id="traps" className={cls("traps")} onClick={() => onToggle("traps")} style={{ color: fill("traps") }}>
...
function BackBody({ selected, onToggle }: { selected: Set<string>; onToggle: (id: string) => void }) {
  const active = (id: string) => selected.has(id);
  const fill = (id: string) => active(id) ? MUSCLE_META[id]?.color ?? "#ef4444" : "currentColor";
  const cls = (id: string) =>
    `cursor-pointer transition-all duration-200 ${active(id) ? "opacity-100 drop-shadow-md" : "opacity-20 hover:opacity-40"}`;
  ...
  return (
    <svg viewBox="0 0 660.46 1206.46" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-full w-auto max-h-[500px] md:max-h-[600px]">
      <defs>
        <radialGradient id="jointradial-b" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
          <stop offset="0%" style={{ stopColor: "rgb(254,91,127)", stopOpacity: 1 }} />
          <stop offset="100%" style={{ stopColor: "rgb(231,236,239)", stopOpacity: 1 }} />
        </radialGradient>
      </defs>

      <g id="traps-back" className={cls("traps")} onClick={() => onToggle("traps")} style={{ color: fill("traps") }}>
      ...
      <g id="calves-back" className={cls("calves")} onClick={() => onToggle("calves")} style={{ color: fill("calves") }}>
      ...
      <g id="hamstrings" className={cls("hamstrings")} onClick={() => onToggle("hamstrings")} style={{ color: fill("hamstrings") }}>
      ...
      <g id="glutes" className={cls("glutes")} onClick={() => onToggle("glutes")} style={{ color: fill("glutes") }}>
      ...
      <g id="forearms-back" className={cls("forearms")} onClick={() => onToggle("forearms")} style={{ color: fill("forearms") }}>
      ...
      <g id="triceps" className={cls("triceps")} onClick={() => onToggle("triceps")} style={{ color: fill("triceps") }}>
      ...
      <g id="lats" className={cls("lats")} onClick={() => onToggle("lats")} style={{ color: fill("lats") }}>
      ...
      <g id="lowerback" className={cls("lowerback")} onClick={() => onToggle("lowerback")} style={{ color: fill("lowerback") }}>
      ...
      <g id="traps-middle-back" className={cls("traps-middle")} onClick={() => onToggle("traps-middle")} style={{ color: fill("traps-middle") }}>
      ...
      <g id="rear-shoulders" className={cls("rear-shoulders")} onClick={() => onToggle("rear-shoulders")} style={{ color: fill("rear-shoulders") }}>
      ...
      <g className="pointer-events-none opacity-40">
        ...
        <path d="M330.26,131.3c15.78,0,25.12,12.03,29.48,9.51,4.35-2.51,8.07-13.01,16.41-23.77,8.34-10.76-.36-12.56,9.08-42.7,9.43-30.14-14.51-56.46-20.68-60.41-6.17-3.95,17.41,10.04,4.72-3.23-12.7-13.27-38.46-4.3-46.44-2.15s27.21-9.69,0-6.1c-22.12,2.92-39.92,14.6-46.79,34.66" fill="none" stroke="#d0d4e0" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3.52"/>
        <path d="M330.26,131.3c-15.79,0-25.13,12.03-29.48,9.51-4.36-2.51-8.08-13.01-16.42-23.77s.36-12.56-9.07-42.7c-1.49-4.74-2.14-17.9-2.17-22.01v-.04c-.04-5.66,1.11-10.73,2.93-15.18" fill="none" stroke="#d0d4e0" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3.52"/>
      </g>
    </svg>
  );
}

function getMuscleIds(muscleId: string): string[] {
  return MUSCLE_ID_MAP[muscleId.toLowerCase()] || [muscleId.toLowerCase()];
}

// Find the API muscle whose NAME maps to a given SVG muscle ID
function findMuscleByName(muscleList: MuscleItem[], svgId: string): MuscleItem | undefined {
  return muscleList.find(m => getMuscleIds(m.name).includes(svgId));
}

export default function BodyMap({ muscleList, selectedMuscles, onToggleMuscle }: BodyMapProps) {
  const [view, setView] = useState<"both" | "front" | "back">("both");

  // Expand selected API muscle UUIDs → SVG IDs via muscle NAME
  const selectedSet = new Set<string>();
  selectedMuscles.forEach(apiId => {
    const muscle = muscleList.find(m => m.id === apiId);
    if (muscle) {
      getMuscleIds(muscle.name).forEach(svgId => selectedSet.add(svgId));
    }
  });

  const toggleMuscle = (svgId: string) => {
    const mappedIds = getMuscleIds(svgId);
    mappedIds.forEach(mappedId => {
      // Match by muscle NAME, not by UUID
      const apiMuscle = findMuscleByName(muscleList, mappedId);
      if (apiMuscle) {
        onToggleMuscle(apiMuscle.id);
      }
    });
  };
  ...
  const selectGroup = (muscles: string[]) => {
    muscles.forEach(muscleId => {
      const mappedIds = getMuscleIds(muscleId);
      mappedIds.forEach(mappedId => {
        const apiMuscle = findMuscleByName(muscleList, mappedId);
        if (apiMuscle && !selectedMuscles.includes(apiMuscle.id)) {
          onToggleMuscle(apiMuscle.id);
        }
      });
    });
  ...
  const getMuscleColor = (id: string): string => {
    // id might be a UUID — resolve to name first
    const muscle = muscleList.find(m => m.id === id);
    const name = muscle ? muscle.name.toLowerCase() : id.toLowerCase();
    const svgIds = getMuscleIds(name);
    for (const svgId of svgIds) {
      if (MUSCLE_META[svgId]) return MUSCLE_META[svgId].color;
    }
    return "#64748b";
  };

  const getMuscleLabel = (id: string): string => {
    const item = muscleList.find(m => m.id === id);
    if (item) return item.name;
    
    const svgIds = getMuscleIds(id.toLowerCase());
    for (const svgId of svgIds) {
      if (MUSCLE_META[svgId]) return MUSCLE_META[svgId].label;
    }
    return id;
  };
      ...
      <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-2">
          {Object.entries(MUSCLE_GROUPS).map(([group, muscles]) => (
            <button
              key={group}
              onClick={() => selectGroup(muscles)}
              className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition-all hover:border-primary hover:text-primary dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300"
            >
              + {group}
            </button>
          ))}
          {selectedMuscles.length > 0 && (
            <button
              onClick={clearAll}
              className="rounded-full border border-red-500 bg-transparent px-3 py-1.5 text-xs font-medium text-red-500 transition-all hover:bg-red-50"
            >
              Clear all
            </button>
          )}
      ...
      <div className="flex justify-center gap-4">
        {(view === "both" || view === "front") && (
          <div className="flex flex-1 flex-col items-center">
            <span className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Front</span>
            <div className="flex h-[400px] w-full items-center justify-center text-slate-300 sm:h-[500px]">
              <FrontBody selected={selectedSet} onToggle={toggleMuscle} />
            </div>
          </div>
        )}
        {(view === "both" || view === "back") && (
          <div className="flex flex-1 flex-col items-center">
            <span className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Back</span>
            <div className="flex h-[400px] w-full items-center justify-center text-slate-300 sm:h-[500px]">
              <BackBody selected={selectedSet} onToggle={toggleMuscle} />
            </div>
          </div>
        )}
      </div>
      ...
      {selectedMuscles.length > 0 && (
        <div className="mt-6 border-t border-slate-200 pt-4 dark:border-slate-700">
          <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400">
            Selected — {selectedMuscles.length} muscle{selectedMuscles.length !== 1 ? "s" : ""}
          </p>
          <div className="flex flex-wrap gap-2">
            {selectedMuscles.map((id) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-semibold transition-all hover:opacity-80"
                  style={{
