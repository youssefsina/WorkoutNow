...
export default function HistoryPage() {
  ...
  const fetchHistory = async (pageNum: number) => {
    try {
      ...
      const payload = res.data as { data?: WorkoutSession[]; total?: number } | WorkoutSession[];
  ...
  return (
    <div className="animate-fadeUp space-y-6">
      {/* ── Header Banner ─────────────────────────── */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-blue-50/10 to-indigo-500/10 border border-primary/20 p-6 md:p-8">
        <div className="pointer-events-none absolute -right-12 -top-12 h-56 w-56 rounded-full bg-primary/10 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-8 left-16 h-32 w-32 rounded-full bg-indigo-500/20 blur-2xl" />
        <div className="relative">
          ...
          <p className="mt-1 text-muted-foreground">All your completed workout sessions</p>

          {!loading && sessions.length > 0 && (
            <div className="mt-5 flex flex-wrap gap-3">
              <div className="flex items-center gap-3 rounded-xl bg-card px-4 py-2.5 shadow-sm border border-border">
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10">
                  <span className="material-symbols-outlined text-lg text-primary">fitness_center</span>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Total Sessions</p>
                  <p className="text-lg font-extrabold text-foreground">{totalSessions}</p>
                </div>
              </div>
              {totalMinutes > 0 && (
                <div className="flex items-center gap-3 rounded-xl bg-card px-4 py-2.5 shadow-sm border border-border">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-5 rounded-2xl border-2 border-dashed border-border bg-card py-20 text-center">
          <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-muted">
            <span className="material-symbols-outlined text-4xl text-muted-foreground">fitness_center</span>
          </div>
          <div>
            <p className="text-xl font-bold text-foreground">No workouts yet</p>
            <p className="mt-1 text-sm text-muted-foreground">Complete a workout to start tracking your progress</p>
          </div>
          <Link
          ...
          {Object.entries(grouped).map(([date, workouts]) => (
            <div key={date}>
              ...
              <div className="mb-3 flex items-center gap-3">
                ...
                <span className="text-xs text-muted-foreground">{workouts.length} session{workouts.length !== 1 ? "s" : ""}</span>
              </div>
              ...
              <div className="space-y-2.5">
                {workouts.map((session, idx) => (
                  <div
                    key={session.id}
                    className="animate-fadeUp overflow-hidden rounded-xl border border-border bg-card shadow-sm transition hover:border-primary/20 hover:shadow-md"
